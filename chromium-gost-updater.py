...
class PackageVersions:
    ...
    def differ(self) -> bool:
        ...
        log_debug(f"differ: local={self.__local}, remote={self.__remote}, differ={result}")
        return result
...
class Config():
    ...
    def __load_config(self) -> None:
        ...
        try:
            ...
            print("Не удалось распарсить настройки, используем настройки по умолчанию:", e)
            return
...
class Downloader:
    ...
    def download_package(self, version: str) -> Path | None:
        ...
        url = self.PACKAGE_DOWNLOAD_URL_TEMPLATE.replace("{{version}}", version).replace("{{extension}}", ext)
...
def cleanup_old_package_files(keep_current: str | None = None) -> None:
    ...
    try:
        old_packages = chain(
            tmp_dir.glob("chromium-gost-*.deb"),
            tmp_dir.glob("chromium-gost-*.rpm")
        )
    except Exception:
        return
    ...
    if display:
        try:
            ...
            if "DBUS_SESSION_BUS_ADDRESS" in os.environ:
                env_vars["DBUS_SESSION_BUS_ADDRESS"] = os.environ["DBUS_SESSION_BUS_ADDRESS"]
            
            env_args = []
            for k, v in env_vars.items():
                env_args.extend([f"--setenv={k}={v}"])
            
            cmd = ["systemd-run", "--user", "--collect", "--no-block"] + env_args + [script_path]
...
CHROMIUM_GOST_UPDATER_FORUM_URL = "https://www.cryptopro.ru/forum2/default.aspx?g=posts&t=9991&find=lastpost"
...
class GuiBackend:
    ...
    def _find_icon_path(self) -> Path | None:
        ...
        share_icon = Path.home() / ".local" / "share" / "chromium-gost-updater" / "chromium-gost-logo.png"
    ...
    def create(cls) -> "GuiBackend":
        ...
        try:
            ...
            try:
                # Тестируем успешность импортов. Импорты ниже нужны
                from PyQt5.QtWidgets import QApplication, QSystemTrayIcon, QMenu, QAction, QMessageBox # pyright: ignore[reportUnusedImport]
...
class QtBackend(GuiBackend):
    ...
    def create_tray(self, updater_app: UpdaterApp) -> None:
        ...
        check_action.triggered.connect(lambda checked=False: threading.Thread(target=updater_app.manual_check_and_notify, daemon=True).start())
        forum_action.triggered.connect(lambda checked=False: threading.Thread(target=updater_app.show_forum, daemon=True).start())
        quit_action.triggered.connect(lambda checked=False: self.quit())
        tray.activated.connect(lambda reason: self.__consider_on_tray_activated(updater_app, reason))
        self.tray = tray
        tray.show()
        ...
        self.__dialog_signaler.show_dialog_signal.connect(lambda: self.__show_update_dialog_impl(updater_app))
    ...
    def show_update_dialog(self, updater_app: UpdaterApp) -> None:
        ...
        if not self.app:
            ...
            if is_main_thread:
                ...
                log_debug(f"show_update_dialog: calling _show_update_dialog_impl directly from main thread")
            else:
                # Вызываем из главного потока через сигнал Qt
                log_debug(f"show_update_dialog: scheduling dialog in main thread via Qt signal")
    ...
    def __show_update_dialog_impl(self, updater_app: UpdaterApp) -> None:
        ...
        log_debug(f"_show_update_dialog_impl: Qt dialog result={result}, clicked={clicked}")
        if clicked == update_btn:
            ...
            log_debug(f"_show_update_dialog_impl: user clicked Remind Later or closed dialog")
...
class AppIndicatorGuiBackend(GuiBackend):
    ...
    def create_tray(self, updater_app: UpdaterApp) -> None:
        ...
        check_item.connect("activate", lambda ignored_widget: threading.Thread(target=updater_app.manual_check_and_notify, daemon=True).start())
    ...
    def __do_show_tray_message(self, message: str, timeout: int = 5000) -> None:
        ...
        notification = Notify.Notification.new(
            APPNAME,
            message,
            icon_uri if icon_uri else None
        )
...
class NoneGuiBackend(GuiBackend):
    ...
    def create_tray(self, updater_app) -> None:
        ...
        print("GUI недоступен; запущен в headless режиме. Установите PySide6/PyQt5 или python3-gi для работы с треем.")
...
class UpdaterAppImpl(UpdaterApp):
    ...
    def has_updates(self) -> bool:
        ...
        if remote_version in self.state.get("ignored_versions", []):
            log_debug(f"has_updates: {remote_version} is in ignored_versions, returning False")
            return False
    ...
    def cleanup_installed_version(self) -> None:
        ...
        for version in list(ignored_versions):
            ...
            if normalized_version == local_version:
                ...
                log_debug(f"cleanup_installed_version: removed {version} from ignored_versions")
        ...
        for version in list(remind_at.keys()):
            ...
            if normalized_version == local_version:
                ...
                log_debug(f"cleanup_installed_version: removed {version} from remind_at")
    ...
    def do_update(self) -> None:
        ...
        if not package_path:
            ...
            GUI_BACKEND.show_tray_message(f"Не удалось скачать {remote_version} после {retries} попыток", 5000)
            return
        ...
        for attempt in range(1, attempts+1):
            ...
            if ok:
                ...
                if attempt < attempts:
                    GUI_BACKEND.show_tray_message(f"Ошибка установки (попытка {attempt}/{attempts}). Попробуйте снова.", 4000)
                    time.sleep(1.5)
                else:
                    GUI_BACKEND.show_tray_message(f"Установка не удалась: {out[:200]}", 8000)
                    return
    ...
    def manual_check_and_notify(self) -> None:
        ...
        log_debug(f"manual_check_and_notify: state={json.dumps(self.state, ensure_ascii=False)}")
        ...
        if remote in ignored_versions:
            ...
            if differ:
                log_debug(f"manual_check_and_notify: versions differ, showing dialog (ignored version)")
                self.show_update_dialog()
            else:
                log_debug("manual_check_and_notify: versions same, showing no updates message")
                GUI_BACKEND.show_tray_message("Обновлений не найдено")
        elif differ:
...
def main() -> None:
    ...
    if gui_available and not check_only:
        ...
        log_debug(f"main: has_updates={has_updates}, show_tray_lazily={show_tray_lazily}")
        ...
        if not has_updates and show_tray_lazily:
            log_debug("main: No updates available and lazy mode enabled, exiting without showing tray")
            return
