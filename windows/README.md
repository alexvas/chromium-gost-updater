...
При наличии новой версии обновлятор скачивает инсталлер с сервера КриптоПро в фоне и уведомляет в трее. Проверяется только структура PE (MZ/заголовок); подпись Authenticode и хеш с сервера не проверяются. Проводник в папке кэша (`%LOCALAPPDATA%\chromium_gost_updater\packages`) открывается только по явному действию — клик по иконке в трее или «Проверить сейчас». Установку браузера пользователь выполняет вручную, запустив скачанный `.exe`.
