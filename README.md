...
Это обновлятор браузера Chromium Gost сборки КриптоПро. Скрипт на Питоне, который следит за выходом новой версии браузера, уведомляет об этом пользователя в системном трее, скачивает дистрибутив в фоне и по явному действию (клик по иконке или «Проверить сейчас») показывает команду для ручной установки (`sudo apt install …` / `sudo dnf install …`). Скрипт рассчитан на rpm и deb дистрибутивы Linux; для Windows см. [windows/README.md](windows/README.md).
